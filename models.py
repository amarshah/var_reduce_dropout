...
def apply_layer(input, layer, batch_norm, dropout_mask = None):
	if dropout_mask is None:
		output = layer(input)
	else:
		output = layer(dropout_mask(input))

	if batch_norm: output = BatchNormalization()(output)
...
def define_model(n_in, n_layer, n_out, p, dropout_flag, batch_norm):
	...
	# apply model
	out1 = apply_layer(x, layer1, batch_norm, dropout_in)
	out1 = apply_layer(out1, layer2, batch_norm, dropout_layer1)
	out1 = apply_layer(out1, layer3, batch_norm, dropout_layer2)
	out1 = apply_layer(out1, layer4, batch_norm, dropout_layer3)
	out1 = apply_layer(out1, softmax_layer, batch_norm, dropout_layer4)

	if dropout_flag != 1:
		out2 = apply_layer(x, layer1, batch_norm, dropout_neg_in)
		out2 = apply_layer(out2, layer2, batch_norm, dropout_neg_layer1)
		out2 = apply_layer(out2, layer3, batch_norm, dropout_neg_layer2)
		out2 = apply_layer(out2, layer4, batch_norm, dropout_neg_layer3)
		out2 = apply_layer(out2, softmax_layer, batch_norm, dropout_neg_layer4)
...
def run_model(n_in, n_layer, n_out, p, dropout_flag, batch_norm,
	n_batch, n_epoch, n_mc, X_train, Y_train, X_test, Y_test):

	model = define_model(n_in, n_layer, n_out, p, dropout_flag, batch_norm)
